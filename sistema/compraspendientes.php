...
<button type="button" class="close" data-dismiss="modal" aria-hidden="true"><img src="assets/images/close.png"/></button> 
                    <h3 class="panel-title"><i class="fa fa-clipboard"></i> Factura de Compra</h3> 
                                                        </div> 
                                                        <div class="panel-body"> 
                                                         <div id="muestracomprasmodal"></div>
                                                        </div>
...
<div class="row">
<div class="col-sm-12">
<div class="page-header-title"><h4 class="pull-left page-title"><i class="fa fa-tasks"></i> Cuentas por Pagar</h4>
...
<div class="panel-heading">
<h3 class="panel-title"><i class="fa fa-tasks"></i> Cuentas por Pagar<span class="pull-right">
...
<li><a href="reportepdf?tipo=<?php echo base64_encode("COMPRASXPAGAR") ?>" target="_blank" rel="noopener noreferrer" data-toggle="tooltip" data-placement="left" title="" data-original-title="Listado (Pdf)"><i class="fa fa-file-pdf-o"></i> Listado (Pdf)</a></li>
<li><a href="reporteexcel?tipo=<?php echo base64_encode("COMPRASXPAGAR") ?>" data-toggle="tooltip" data-placement="left" title="" data-original-title="Listado (Excel)"><i class="fa fa-file-excel-o"></i> Listado (Excel)</a></li>
            </ul>
</div>
</span></h3>
</div>
		...
		if(isset($_GET["mesage"]))
{
	switch($_GET["mesage"])
	{
		...
		echo "<center><span class='fa fa-check-square-o'></span> LA COMPRA PENDIENTE FUE PAGADA AL PROVEEDOR EXITOSAMENTE </center>"; 
        echo "</div>";
		break;
		...
		echo "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
		echo "<center><span class='fa fa-info-circle'></span> USTED NO TIENE ACCESO PARA PAGAR COMPRAS PENDIENTES, NO ERES EL ADMINISTRADOR DEL SISTEMA</center>";
        echo "</div>";  
		break;
...
<td><?php echo "<strong>".$config[0]['simbolo']."</strong>".number_format($reg[$i]['subtotalivasic'], 2, '.', ','); ?></td>
<td><?php echo "<strong>".$config[0]['simbolo']."</strong>".number_format($reg[$i]['subtotalivanoc'], 2, '.', ','); ?></td>
<td><?php echo "<strong>".$config[0]['simbolo']."</strong>".number_format($reg[$i]['totalivac'], 2, '.', ','); ?></td>
<td><?php echo "<strong>".$config[0]['simbolo']."</strong>".number_format($reg[$i]['totalc'], 2, '.', ','); ?></td>
...
elseif($reg[$i]['fechavencecredito'] < date("Y-m-d")) { echo Dias_Transcurridos(date("Y-m-d"),$reg[$i]['fechavencecredito']); } ?></td>
											   <td>
<a href="#" class="btn btn-success btn-xs" onClick="VerCompra('<?php echo base64_encode($reg[$i]['codcompra']) ?>')" data-href="#" data-toggle="modal" data-target=".bs-example-modal-lg" data-placement="left" data-backdrop="static" data-keyboard="false" data-id="" rel="tooltip" data-original-title="Ver Compra" title="Ver Producto"><i class="fa fa-search-plus"></i></a>
 
<?php if($reg[$i]['statuscompra']=="PENDIENTE") { ?><a class="btn btn-info btn-xs" data-toggle="tooltip" data-placement="left" title="" data-original-title="Pagar Factura" onClick="pagar('eliminar?<?php echo base64_encode('eliminar'); ?>&codcompra=<?php echo base64_encode($reg[$i]["codcompra"]) ?>&tipo=<?php echo base64_encode("PAGARFACTURA") ?>')"><i class="fa fa-refresh"></i></a><?php } ?>

<a href="reportepdf?codcompra=<?php echo base64_encode($reg[$i]['codcompra']); ?>&tipo=<?php echo base64_encode("FACTURACOMPRAS") ?>" target="_black" class="btn btn-warning btn-xs" data-toggle="tooltip" data-placement="left" title="" data-original-title="Factura de Compra"><i class="fa fa-print"></i></a></td>
											   </tr>
											   <?php } } ?>
											   </tbody>
</table>
