...
<button type="button" class="close" data-dismiss="modal" aria-hidden="true"><img src="assets/images/close.png"/></button> 
         <h3 class="panel-title"><i class="fa fa-align-justify"></i> Datos de Mov. de Caja</h3> 
                                                        </div> 
                                                        <div class="panel-body"> 
                                                         <div id="muestramovimientocajamodal"></div>
                                                        </div>
...
<div class="row">
<div class="col-sm-12">
<div class="page-header-title"><h4 class="pull-left page-title"><i class="fa fa-tasks"></i> Control de Mov. de Cajas</h4>
<ol class="breadcrumb pull-right"><li><a href="panel">Inicio</a></li>
<li class="active">Control de Mov. de Cajas</li>
</ol>
...
<div class="panel-heading">
<h3 class="panel-title"><i class="fa fa-tasks"></i> Movimientos<span class="pull-right"><a href="formovimientocaja" class="btn btn-default dropdown-toggle" data-toggle="tooltip" data-placement="left" title="" data-original-title="Nuevo Mov. de Caja"><span class="fa fa-plus"></span> Nuevo</a></span></h3></div>
		...
		if(isset($_GET["mesage"]))
{
	switch($_GET["mesage"])
	{
		...
		echo "<span class='fa fa-check-square-o'></span> EL MOVIMIENTO DE CAJA FUE ELIMINADO EXITOSAMENTE</center>";
        echo "</div>";  
		break;
		...
		echo "<center><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
		echo "<span class='fa fa-info-circle'></span> ESTE MOVIMIENTO DE CAJA NO PUEDE SER ELIMINADO, SE ENCUENTRA DESACTIVADO</center>";
        echo "</div>";   
		break;
		...
		echo "<center><span class='fa fa-info-circle'></span> USTED NO TIENE ACCESO PARA ELIMINAR MOVIMIENTO DE CAJA, NO ERES EL ADMINISTRADOR DEL SISTEMA </center>";  
        echo "</div>"; 
		break;
...
?>
											   ...
											   <td><?php echo "<strong>".$config[0]['simbolo']."</strong>".$reg[$i]['montomovimientocaja']; ?></td>
											   <td><?php echo $reg[$i]['fechamovimientocaja']; ?></td>
											   <td>
 
 <a href="#" class="btn btn-success btn-xs" data-placement="left" title="Ver Movimiento" data-original-title="" data-href="#" data-toggle="modal" data-target="#panel-modal" data-backdrop="static" data-keyboard="false" onClick="VerMovimientoCaja('<?php echo base64_encode($reg[$i]["codmovimientocaja"]); ?>')"><i class="fa fa-search-plus"></i></a>
								  
<a href="#" class="btn btn-primary btn-xs" data-toggle="tooltip" data-placement="left" title="" data-original-title="Editar Movimiento" onClick="actualizar('formovimientocaja?codmovimientocaja=<?php echo base64_encode($reg[$i]["codmovimientocaja"]) ?>')"><i class="fa fa-pencil"></i></a>
                                 
<?php if(date("Y-m-d",strtotime($reg[$i]['fechamovimientocaja']))!=date("Y-m-d")){ ?>

<a href="#" class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="left" title="" data-original-title="Eliminar Movimiento" onClick="VerificaMovimiento()"><i class="fa fa-trash-o"></i></a>

<?php } else { ?>

<a href="#" class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="left" title="" data-original-title="Eliminar Movimiento" onClick="eliminar('eliminar?<?php echo base64_encode('eliminar'); ?>&codmovimientocaja=<?php echo base64_encode($reg[$i]["codmovimientocaja"]) ?>&codcaja=<?php echo base64_encode($reg[$i]["codcaja"]) ?>&codmovimientocaja=<?php echo base64_encode($reg[$i]["codmovimientocaja"]) ?>&tipomovimientocaja=<?php echo base64_encode($reg[$i]["tipomovimientocaja"]) ?>&montomovimientocaja=<?php echo base64_encode($reg[$i]["montomovimientocaja"]) ?>&tipo=<?php echo base64_encode("MOVIMIENTOCAJA") ?>')"><i class="fa fa-trash-o"></i></a>
