...
<cbc:PriceAmount currencyID="'.$cabecera["COD_MONEDA"].'">'.$detalle[$i]["txtPRECIO_DET"].'</cbc:PriceAmount>
    ...
    public function obtener_cdr($data_comprobante, $rutas) {
        
        $ruc = $data_comprobante['EMISOR_RUC'];
        $tipo_comprobante = $data_comprobante['TIPO_COMPROBANTE'];
        $serie_comprobante = $data_comprobante['SERIE_COMPROBANTE'];
        $numero_comprobante = $data_comprobante['NUMERO_COMPROBANTE'];
        
        $usuario_sol = $data_comprobante['EMISOR_USUARIO_SOL'];
        $pass_sol = $data_comprobante['EMISOR_PASS_SOL'];

        $ruta_archivo_cdr = $rutas['ruta_cdr'];
        $archivo = $rutas['nombre_archivo'];
        $soapUrl = $rutas['ruta_ws'];
        ...
        <soapenv:Body>
            <ser:getStatusCdr>
                ...
                <serieComprobante>'.$serie_comprobante.'</serieComprobante>
                <numeroComprobante>'.$numero_comprobante.'</numeroComprobante>
            </ser:getStatusCdr>
        </soapenv:Body>
        </soapenv:Envelope>';
        ...
        if ($httpcode == 200) {
            ...
            if (isset($doc->getElementsByTagName('content')->item(0)->nodeValue)) {
                ...
                $resp['mensaje'] = $doc->getElementsByTagName('statusMessage')->item(0)->nodeValue;
                $resp['hash_cdr'] = "";
            }
        } else {
            //echo "no responde web";
            $resp['respuesta'] = 'error';
            $resp['cod_sunat'] = "0000";
            $resp['mensaje'] = "Codigo de Error: 0000 <br /> Web Service de Prueba SUNAT - Fuera de Servicio: <a href='".$soapUrl."' target='_blank'>".$soapUrl."</a>, Para validar la información llamar al: *4000 (Desde Claro, Entel y Movistar) - SUNAT";
            $resp['hash_cdr'] = "$httpcode";
        }
        return $resp;
    }
